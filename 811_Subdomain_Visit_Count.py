...
class Solution:
    def subdomainVisits(self, cpdomains):
        ...
        for each in cpdomains:
            ...
            tmp2 = tmp[1].split('.')
            hash_table[tmp2[-1]] = hash_table.get(tmp2[-1], 0) + int(tmp[0])
            hash_table[tmp2[-2] + '.' + tmp2[-1]] = hash_table.get(tmp2[-2] + '.' + tmp2[-1], 0) + int(tmp[0])
            if len(tmp2) > 2:
                hash_table[tmp[1]] = hash_table.get(tmp2[1], 0) + int(tmp[0])
    ...
    def subdomainVisits2(self, cpdomains):
        ...
        for cpdomain in cpdomains:
            tmpS1 = cpdomain.split()
            tmpS2 = tmpS1[1].split('.')
            tmpS3 = []
            for i in range(len(tmpS2)):
                tmpS3.append('.'.join(tmpS2[i:]))

            for k in tmpS3:
                hash_table[k] = hash_table.get(k, 0) + int(tmpS1[0])
